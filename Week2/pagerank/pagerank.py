...
def transition_model(corpus, page, damping_factor):
    ...
    if(page_len == 0):
        for i in corpus.keys():
            probability_distribution[i] = 1/dict_len
    else:
        factor = (1 - damping_factor)/page_len

        for i in corpus.keys():
            if i in corpus[page]:
                probability_distribution[i] = factor + (damping_factor/page_len)
            else:
                probability_distribution[i] = factor
...
def iterate_pagerank(corpus, damping_factor):
    ...
    while 1:
        for i in corpus:
            ...
            for j in corpus:
                if i in corpus[j]:
                    temp += (old[j] / len(corpus[j]))
                if len(corpus[j]) == 0:
                    temp += (old[j]) / len(corpus)
            temp *= damping_factor
            temp += (1 - damping_factor) / pages_num

            new[i] = temp

        diff = max([abs(new[x] - old[x]) for x in old])
        if diff < 0.001:
            break
        else:
            old = new
